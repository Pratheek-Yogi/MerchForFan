...
const BasketballPage = () => {
  ...
  const fetchBasketballProducts = async () => {
    try {
      ...
      if (data.success) {
        ...
        console.error('Failed to fetch basketball products');
      }
    } catch (error) {
  ...
  if (loading) {
    return <div className="loading">Loading basketball products...</div>;
  }

  return (
    <div className="cricket-page-container">
      <div className="cricket-page-header">
        <h1>Basketball Merchandise</h1>
        <p>Official jerseys, fanwear, and accessories for every basketball fan.</p>
      </div>
